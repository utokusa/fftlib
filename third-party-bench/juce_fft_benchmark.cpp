...
std::vector<juce::dsp::Complex<float>> convertToJuceComplexVec(
    std::vector<std::complex<double>> input_buf) {
  auto n = input_buf.size();
  std::vector<juce::dsp::Complex<float>> ret(n);
  for (size_t i = 0; i < n; i++) {
    ret[i] = {static_cast<float>(input_buf[i].real()),
              static_cast<float>(input_buf[i].imag())};
  }
  return ret;
}
...
TEST_CASE("JUCE benchmark (float)", "[!benchmark]") {
  BENCHMARK_ADVANCED("fft 8")(Catch::Benchmark::Chronometer meter) {
    ...
    juce::dsp::FFT fft(3);
    meter.measure(
        [&]() { fft.perform(input_buf.data(), output_buf.data(), false); });
  };
